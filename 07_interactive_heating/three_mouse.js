...
function process_mouse_event(event_name, event){
	...
	if ( obj != "") {
		if((mouse.object != "")&&(mouse.object != obj.name)){
			...
			send_custom_event("mesh_mouse_exit",{ type: obj.userData.type, name: mouse.object});
		...
		send_custom_event(event_name,{ type: obj.userData.type, name: mouse.object});
	}
	else{
		if(mouse.is_inside_object){
			mouse.is_inside_object = false;
...
function onTouch(event){
	...
	if(event.type == "touchstart"){
		...
		if ( obj != "") {
			send_custom_event("mesh_touch_start",{ type: obj.userData.type, name: obj.name});
		}
	}
}
